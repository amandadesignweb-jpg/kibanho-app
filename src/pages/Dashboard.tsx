...
export function Dashboard() {
  ...
  // Próximos banhos (ainda não realizados) primeiro, ordenados por horário; realizados por último.
  const agendamentosHojeOrdenados = [...agendamentosHoje].sort((a, b) => {
        ...
        <>
          <div className="grid shrink-0 grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <Card tone="terracota" className="p-3">
              ...
              <div className="mt-1 text-[20px] font-extrabold text-terracota-dark">
                {formatMoney(valorPendente)}
              </div>
              <TagPill tone="terracota" className="mt-2">
            ...
            <Card className="flex items-center gap-3 p-3">
              ...
              >
                <div className="flex h-[36px] w-[36px] items-center justify-center rounded-full bg-card text-[11px] font-extrabold">
                  {ocupacaoPct}%
                </div>
              </div>
              <div className="text-[10px] font-extrabold uppercase tracking-wider text-text-faint">
          ...
          <div className="grid shrink-0 grid-cols-1 gap-3 md:grid-cols-3">
              ...
              {pendencias.map((a) => (
                <div key={a.id} className="flex flex-wrap items-center justify-between gap-1 border-b border-terracota-border py-[6px] last:border-none">
                  ...
                  <div className="flex items-center gap-[6px]">
                    <div className="text-[12px] font-extrabold text-terracota-dark">
                      {formatMoney(a.valor ?? 0)}
                    </div>
                    <button
                      onClick={() => marcarCobrancaPaga(a.id)}
                      className="rounded-pill bg-gradient-to-br from-blue to-blue-dark px-[9px] py-1 text-[9px] font-extrabold text-white"
                    >
                      Pago
                    </button>
                  </div>
            ...
            <Card className="max-h-[150px] overflow-y-auto p-3">
              <div className="mb-2 flex items-center justify-between">
                <div className="text-[12px] font-extrabold">Boletos vencendo</div>
                <TagPill tone="terracota">{boletos.length}</TagPill>
              </div>
              {boletos.length === 0 && (
                <div className="text-[11.5px] text-text-muted">Nenhum boleto no radar.</div>
              )}
              {boletos.map((b) => {
                const estado = boletoEstado(b)
                return (
                  <div key={b.id} className="border-b border-[#f0ebe0] py-[6px] last:border-none">
                    <div className="flex items-center justify-between">
                      <div className="text-[11.5px] font-bold">{b.nome}</div>
                      <div className="text-[12px] font-extrabold">{formatMoney(b.valor)}</div>
                    </div>
                    <div
                      className={
                        'mt-[1px] text-[10px] font-bold ' +
                        (estado === 'atrasado' ? 'text-terracota-strong' : 'text-terracota')
                      }
                    >
                      {estado === 'atrasado'
                        ? `Seu boleto ${vencimentoLabel(b.data_vencimento).toLowerCase()}`
                        : `Prestes a vencer · ${ESTADO_LABEL[estado]}`}
                    </div>
                    {adiandoId === b.id ? (
                      <div className="mt-[5px] flex items-center gap-[5px]">
                        <input
                          ...
                          autoFocus
                          className="rounded-lg border border-border px-2 py-1 text-[10px] outline-none focus:border-blue"
                        />
                        <button
                          onClick={() => confirmarAdiamento(b.id)}
                          disabled={!novaData}
                          className="rounded-pill bg-gradient-to-br from-blue to-blue-dark px-[9px] py-1 text-[9px] font-extrabold text-white disabled:opacity-50"
                        >
                          Confirmar
                        </button>
                        <button
                          onClick={() => { setAdiandoId(null); setNovaData('') }}
                          className="text-[9px] font-bold text-text-faint"
                        >
                          Cancelar
                        </button>
                      </div>
                      ...
                      <div className="mt-[5px] flex gap-[5px]">
                        ...
                        <button
                          ...
                          className="rounded-pill border border-border px-[9px] py-1 text-[9px] font-extrabold text-text-soft"
                        >
                          Adiado
                        </button>
                      </div>
            ...
            <Card tone="terracota" className="max-h-[150px] overflow-y-auto p-3">
              <div className="mb-2 flex items-center justify-between">
                <div className="text-[12px] font-extrabold">Estoque para repor</div>
                <TagPill tone="terracota">{estoqueAlerta.length}</TagPill>
              </div>
              {estoqueAlerta.length === 0 && (
                <div className="text-[11.5px] text-text-muted">Estoque em dia.</div>
              )}
              {estoqueAlerta.map((p) => (
                <div key={p.id} className="flex items-center justify-between border-b border-terracota-border py-[6px] last:border-none">
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <div className="text-[10.5px] text-text-faint">próximos primeiro, realizados por último</div>
          </div>
          ...
          <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto">
            ...
            {agendamentosHojeOrdenados.map((a) => (
              <div
                key={a.id}
                className={
              ...
              >
                ...
                <div className="min-w-0 flex-grow">
                  ...
                  <div className="mt-[1px] text-[11.5px] text-text-muted">{a.hora.slice(0, 5)}</div>
                </div>
                {a.status === 'realizado' ? (
